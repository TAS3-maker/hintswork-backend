...
// Utility to normalize role name
...
exports.addSponsor = async (req, res) => {
  try {
    ...
    if (!(await isAdminOrSuperAdmin(userId))) {
      return res.status(403).json({ message: "Only Admin/SuperAdmin can add sponsors." });
    }

    const { name, website, description = "", associatedBrands = [] } = req.body;
...
exports.editSponsor = async (req, res) => {
  try {
    const userId = req.user?.id || req.user?.userId;
    if (!(await isAdminOrSuperAdmin(userId))) {
      return res.status(403).json({ message: "Only Admin/SuperAdmin can edit sponsors." });
    }

    const { id } = req.params;
    const { name, website, description, associatedBrands, status } = req.body;
    ...
    if (description !== undefined) sponsor.description = description.trim();
    if (associatedBrands) sponsor.associatedBrands = associatedBrands;
    if (status !== undefined) sponsor.status = status;

    await sponsor.save();
    res.status(200).json({ message: "Sponsor updated successfully", sponsor });
  } catch (error) {
...
exports.deleteSponsor = async (req, res) => {
  try {
    const userId = req.user?.id || req.user?.userId;
    if (!(await isAdminOrSuperAdmin(userId))) {
      return res.status(403).json({ message: "Only Admin/SuperAdmin can delete sponsors." });
    }
...
//
// ✅ Block / Unblock Sponsor (Toggle Status)
//
exports.blockSponsor = async (req, res) => {
  try {
    const userId = req.user?.id || req.user?.userId;
    if (!(await isAdminOrSuperAdmin(userId))) {
      return res.status(403).json({ message: "Only Admin/SuperAdmin can block/unblock sponsors." });
    }
    ...
    sponsor.status = sponsor.status === "Active" ? "Blocked" : "Active";
    await sponsor.save();

    res.status(200).json({ message: `Sponsor ${sponsor.status} successfully`, sponsor });
  } catch (error) {
    console.error("blockSponsor error:", error);
    res.status(500).json({ message: "Server Error", error: error.message });
  }
};
...
exports.getAllSponsors = async (req, res) => {
  try {
    const userId = req.user?.id || req.user?.userId;
    if (!(await isAdminOrSuperAdmin(userId))) {
      return res.status(403).json({ message: "Only Admin/SuperAdmin can view sponsors." });
    }
